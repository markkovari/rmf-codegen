...
interface RustObjectTypeExtensions : ExtensionsBase {
    ...
    fun VrapType.createRustVrapType(): VrapType {
        return when (this) {
            is VrapObjectType -> {
                VrapObjectType(`package` = this.`package`.rustModelFileName(), simpleClassName = this.simpleClassName)
            }

            is VrapEnumType -> {
                VrapEnumType(`package` = this.`package`.rustModelFileName(), simpleClassName = this.simpleClassName)
            }
    ...
    fun List<AnyType>.getTypeInheritance(type: AnyType): List<AnyType> {
        return this.filter { it.type != null && it.type.name == type.name }
    ...
    fun ObjectType.rustStructFields(all: Boolean): List<Property> {
        ...
        return props.filter {
            ((discriminator() == null || it.name != discriminator()) || (it.name == discriminator() && discriminatorValue == null))
        }
    }
