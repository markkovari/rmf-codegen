...
class RequestBuilder constructor(
    ...
    protected fun Resource.constructor(): String {
        val pathArgs = if (!this.fullUri.variables.isEmpty()) {
            this.fullUri.variables.map { it.rustName() }.map { "$it   string" }.joinToString(separator = "\n")
        } else {
            ""
        }
    ...
    private fun Resource.renderMethod(method: Method): String {
        ...
        val methodKwargs = listOf<String>().plus(
            {
                if (bodyVrapType != null && bodyVrapType !is VrapNilType) "body ${
                    method.vrapType()?.rustTypeName()
                }" else ""
            }()
        ).filter {
            it != ""
