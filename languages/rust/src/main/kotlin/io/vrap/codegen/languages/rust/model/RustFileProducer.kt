...
class RustFileProducer constructor(
    ...
    private fun buildModule(moduleName: String, types: List<AnyType>): TemplateFile {
        ...
        val modules = getImports(sortedTypes).map {
            "    \"$it\""
        }
       ...
       """.trimMargin().keepIndentation()
        ...
        val filename = moduleName.rustModelFileName()
        return TemplateFile(content, "src/types/$filename.rs")
    }
    ...
    private fun getImports(types: List<AnyType>): List<String> {
        ...
        objects.filter { it.requiresJsonModule() }.map {
            "encoding/json"
        }
        return objects.asSequence().flatMap {
            it.allProperties.map { property ->
                when (property.type) {
        ...
        }).plus(objects.filter { it.requiresJsonModule() }.map {
            "encoding/json"
    ...
    private fun ObjectType.requiresJsonModule(): Boolean {
        if (allProperties.all { it.isPatternProperty() }) {
            ...
            if (allProperties.map { it.type }.any {
                    when (it) {
    ...
    private fun ObjectType.renderObjectType(): String {
        ...
        if (isMap) {
            ...
            val unmarshalFunc = if (allProperties.map { it.type }.any {
                    when (it) {
                        is ArrayType -> it.items.isDiscriminated()
                        is ObjectType -> it.isDiscriminated()
                        else -> false
                    }
                } || allProperties.any { it.isPatternProperty() }) this.renderUnmarshalFunc() else ""

            val marshalFunc = this.renderMarshalFunc()
            val errorFunc = this.renderErrorFunc()

            val decodeFunc = if (this.discriminatorValue != null && allProperties.any { it.isPatternProperty() }) {
    ...
    fun ObjectType.renderErrorFunc(): String {
        ...
        val messageField = rustStructFields(true).filter {
    ...
    fun ObjectType.renderMarshalFunc(): String {
        if (this.discriminatorValue == null && !allProperties.any { it.isPatternProperty() } && !allProperties.any { it.type is ArrayType && !it.required }) {
            return ""
        }
        ...
        val modifyJsonStmt =
            if (allProperties.any { it.isPatternProperty() } || allProperties.any { it.type is ArrayType && !it.required }) {
                val statements = allProperties.filter { it.type is ArrayType && !it.required }.map {
                    """
                    |   if raw["${it.name}"] == nil {
                    |       delete(raw, "${it.name}")
                    |   }
                    """
                }.plus(allProperties.filter { it.isPatternProperty() }.map {
                    """
    ...
    fun ObjectType.renderUnmarshalFuncPatternHandlers(): String {
        ...
        return allProperties.filter { it.isPatternProperty() }.map {
            """
    ...
    fun ObjectType.renderDiscriminatorMapper(): String {
        val caseStatements = allAnyTypes.getTypeInheritance(this).filterIsInstance<ObjectType>()
            .filter { !it.discriminatorValue.isNullOrEmpty() }.map {
    ...
    fun ObjectType.renderStructFields(): String {
        return rustStructFields(true).map {
    ...
    private fun StringType.renderEnumValues(enumName: String): String {
        return this.enumValues().map { "${enumName}${it.exportName().replace(".", "")} $enumName = \"$it\"" }
